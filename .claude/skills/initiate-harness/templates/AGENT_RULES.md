...
- `blocked` — cannot proceed; `blocked_by` holds the reason. Cleared automatically by `write-bus-input` when the resolving RESEARCH/ADVICE lands, or manually by the user.
- `needs-revision` — plan itself is faulty; halt and surface to the user.

### Phases

1. **Draft** — Converse with the user; finalise PLAN content.
...
- **Interrupted runs:** if the user halts a run mid-execution, their description of what they caught is a hiccup entry — write it to `.claude/skills/_hiccups.md` before resuming.
- **Ambiguity:** if a step is ambiguous, unsafe, or marked [user]: halt, set `status: needs-revision`, surface to the user. Do not improvise.
