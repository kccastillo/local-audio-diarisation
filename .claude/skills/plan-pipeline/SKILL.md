...
<constraints>
- Never write PLAN content to disk yourself. Dispatch `plan-writer` (foreground subagent) for every create-or-update of a PLAN file. The orchestrator may write PLAN *frontmatter* fields (`pipeline_phase`, `audit_state`, `last_executor_outcome`, `verification_state`) directly because those are orchestration-owned — but body content (Objective, Context, Steps, Verification, Executor Notes) flows through `plan-writer` or `plan-executor`.
- Never run `ideate` as a subagent. Invoke `Skill("ideate", ...)` in the parent session.
- Never dispatch more than one phase per invocation. If a synchronous chain would require multiple phase advances (e.g. sufficiency success → immediately dispatch plan-safety), do that within the same invocation only when both stay in the same phase (`drafted`'s audit loop) — phase boundaries (`drafted → checked → executing`) get one transition per invocation, with commit + push between.
