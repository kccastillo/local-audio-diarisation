...
**Step 1 — v1-final tag** — done 2026-05-04.
- Tagged `a57f077` as `v1-final` (annotated). Pushed to origin.
- Recovery: `git checkout v1-final`.

**Step 2 — v2 requirements RESEARCH** — done 2026-05-04. Captured as `Bus/202605040210_RESEARCH_v2-requirements.md`.

Headline finding: operator wants v2 to **simplify** by replacing v1's five-stage pipeline with a single-model wrapper (anchor candidate: WhisperX). Hard constraints unchanged from v1: 8 GB VRAM, all audio/video formats v1 supports, offline. Out-of-scope: stay within v1 scope. No model/library preferences expressed — Step 3 chooses on merit.

**Step 3 — Design decisions PLAN** — done 2026-05-04. Operator delegated full autonomy ("construct on your own"). 14 decisions locked (D1–D14) and recorded in `Bus/202605040220_PLAN_v2-implementation.md` frontmatter `linked_decisions:`.

**Step 4 — Decompose into child PLANs** — collapsed into Step 5. Per CLAUDE.md PLAN-or-not triage, the v2 work fit cleanly into a single 10-step PLAN (`202605040220_PLAN_v2-implementation.md`) rather than 7-9 child PLANs. Decomposition into many child PLANs would have been ceremonial overhead.

**Step 5 — Execute** — done 2026-05-04. PLAN 220 executed end-to-end: `requirements.v2.txt`, `v2/` package (config, preprocessing, pipeline, output, cli), `config/config.v2.yaml`, 24 unit tests passing under v1 venv. README updated with v2 section. v1 untouched.

**Step 6 — Decommission v1 and merge** — DEFERRED. Cannot run safely without hardware validation of v2 on operator's GPU + a real cybersecurity meeting recording. Follow-up PLAN to be drafted at the start of the hardware-validation session.
